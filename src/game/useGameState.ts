...
function getFatigueModifiers(stacks: number) {
  if (stacks >= 3) return { energyCostMult: 1.45, skillsMult: 0.4, happinessGainMult: 0.6 };
  if (stacks === 2) return { energyCostMult: 1.3, skillsMult: 0.6, happinessGainMult: 0.8 };
  if (stacks === 1) return { energyCostMult: 1.15, skillsMult: 0.8, happinessGainMult: 1 };
  return { energyCostMult: 1, skillsMult: 1, happinessGainMult: 1 };
}
...
function calculateSleepStartEnergy(slotsRemaining: number, energyDebt: number): number {
  ...
  return clamp(baseEnergy + bonus - energyDebt, 1, 100);
}
...
function applyAction(state: GameState, action: GameAction): GameState {
  ...
  const effectiveEnergyCost = Math.ceil(Math.max(0, action.energyCost) * fatigue.energyCostMult);
  ...
  const energyDebtAdded = effectiveEnergyCost > state.energy ? effectiveEnergyCost - state.energy : 0;
  ...
  if ((scaledEff.skills ?? 0) > 0) scaledEff.skills = Math.round((scaledEff.skills ?? 0) * fatigue.skillsMult);
  if ((scaledEff.happiness ?? 0) > 0) scaledEff.happiness = Math.round((scaledEff.happiness ?? 0) * fatigue.happinessGainMult);
  ...
  if (energyDebtAdded > 0) {
    next = addLog(next, {
      text: `You forced yourself to keep going.\nYou will wake up exhausted tomorrow (-${energyDebtAdded} energy).`,
      type: "event",
      day: next.day,
    });
  }
  ...
  if (next.slotsRemaining <= 0) {
    return startNextDay(addLog(next, { key: "log.system.noTimeLeft", type: "event", day: next.day }), 70);
  }
