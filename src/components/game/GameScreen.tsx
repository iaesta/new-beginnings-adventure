...
import SalaryBubble from "./SalaryBubble";
...
const PRACTICE_MENU: GameAction = {
  ...
  icon: "🧪",
  label: "Practice",
  description: "Apply what you learned in guided practice.",
  energyCost: 0,
  effects: {},
};
...
function burnoutFireStyle(stacks: number): CSSProperties {
  ...
  if (stacks === 1) return { fontSize: "1.2rem", lineHeight: 1, filter: "drop-shadow(0 0 6px rgba(250,204,21,0.35))" };
  if (stacks === 2) return { fontSize: "1.5rem", lineHeight: 1, filter: "drop-shadow(0 0 8px rgba(249,115,22,0.4))" };
  return { fontSize: "1.8rem", lineHeight: 1, filter: "drop-shadow(0 0 10px rgba(239,68,68,0.45))" };
}
        ...
        <div
          ...
          style={{
            ...
            background: `conic-gradient(
              ...
              rgba(99,102,241,0.28) ${sleepZoneStartDeg}deg,
              rgba(99,102,241,0.28) 360deg
            )`,
            ...
            return (
              <button
                ...
                className={`rounded-xl border px-3 py-2 text-left transition-colors ${
                  willCauseBurnout
                    ? "border-red-300/20 bg-red-500/5 hover:bg-red-500/10"
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  {shownEnergyCost > 0 && (
                    <span className={`text-xs ${willCauseBurnout ? "text-red-100" : "text-white/70"}`}>
                      ⚡ {shownEnergyCost}
                    </span>
                  )}
                </div>
...
const GameScreen = ({ state, availableActions, onAction }: GameScreenProps) => {
  ...
  const studyOptions = useMemo(
    () => availableActions.filter((a) => a.id === "study_1h" || a.id === "study_2h" || a.id === "study_4h").map(tr),
    [availableActions, T]
  );

  const practiceOptions = useMemo(
    () =>
      ACTIONS.filter((a) => a.parentId === "practice" && (!a.minSkills || state.skills >= a.minSkills)).map(tr),
    [T, state.skills]
  );

  const socialOptions = useMemo(
    () => availableActions.filter((a) => a.id.startsWith("socialize_") && a.id !== "socialize").map(tr),
    [availableActions, T]
  );
  ...
  const actionsForGrid = useMemo(() => {
    ...
    if (practiceOptions.length > 0) withMenus.push({ ...PRACTICE_MENU, label: T.MODALS.practiceTitle, description: T.MODALS.practiceSubtitle });
  ...
  }, [availableActions, practiceOptions.length, socialOptions.length, restOptions.length, studyOptions.length, T]);
  ...
  const unlockedAchievements = useMemo(
    () => (!state.milestones?.length ? [] : MILESTONES.filter((m) => state.milestones.includes(m.id))),
    [state.milestones]
  );
  ...
  return (
    <div className="min-h-screen lg:h-screen flex flex-col lg:flex-row">
          ...
          {(state.fatigueStacks > 0 || state.energyDebt > 0) && (
            <div className="flex flex-col items-center gap-1">
              {state.fatigueStacks > 0 && (
                <div style={burnoutFireStyle(state.fatigueStacks)} title={`Burnout ${state.fatigueStacks}`}>
                  🔥
                </div>
              )}
              {state.energyDebt > 0 && (
                <div
                  className="rounded-full border border-red-300/30 bg-red-500/10 px-2 py-1 text-[11px] font-semibold text-red-100"
                  title={`Tomorrow energy penalty: -${state.energyDebt}`}
                >
                  🔥⚡-{state.energyDebt}
                </div>
              )}
          ...
          <div className="space-y-1">
            ...
            {state.energyDebt > 0 && (
              <div className="h-2 w-full rounded-full bg-transparent overflow-hidden">
                <div
                  className="h-full rounded-full bg-red-500/35"
                  style={{ width: `${Math.min(state.energyDebt, 100)}%` }}
                  title={`Tomorrow wake-up penalty: -${state.energyDebt} energy`}
                />
              </div>
            )}
          </div>
      ...
      <div className="flex-1 min-h-0 lg:h-screen flex flex-col">
        <main className="flex-1 min-h-0 p-4 lg:p-6">
          <div className="relative h-full rounded-2xl border border-white/10 bg-white/5 backdrop-blur overflow-hidden">
            <SalaryBubble workedDays={state.workedDaysThisPeriod} />
        ...
        <section className="border-t border-border p-4 lg:p-6 bg-white/5 backdrop-blur">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {actionsForGrid.map((action) => {
              ...
              const workedAlreadyLocked = action.id === "work" && state.workedToday;
              ...
              return (
                <ActionButton
                  ...
                  disabled={workedAlreadyLocked}
                  lowEnergy={willCauseBurnout}
                  willCauseBurnout={willCauseBurnout}
                  onClick={() => handleActionClick(action.id)}
                  onHover={(text, rect) =>
                    showTooltip(
                      workedAlreadyLocked ? "You already worked today." : text,
                      rect
                    )
                  }
                />
      ...
      {tooltip && (
        ...
        >
          <div className="max-w-xs whitespace-pre-line rounded-xl border border-white/10 bg-black/80 backdrop-blur-md px-4 py-2 text-xs text-white shadow-lg">
            {tooltip.text}
          </div>
        </div>
      )}
