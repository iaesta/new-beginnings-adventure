...
const ActionButton = ({
  ...
  const isDisabled = disabled || (lowEnergy && shownEnergyCost > 0);
  ...
  return (
    <button
      ...
      disabled={isDisabled}
      className={`group relative w-full text-left rounded-2xl border p-4 transition-all duration-200 backdrop-blur-sm shadow-[0_4px_20px_rgba(0,0,0,0.25)] ${
        isDisabled
    ...
    >
      <div className="flex items-center gap-3">
        ...
        {shownEnergyCost > 0 && (
          <span
            className={`shrink-0 inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold border ${
              shownEnergyCost > action.energyCost
